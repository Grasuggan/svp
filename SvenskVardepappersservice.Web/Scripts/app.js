    ...
    axios({
        ...
        if (response.data.ResultCount == 0) {
            ...
            for (var i = 0; i < response.data.Result.length; i++) {
                ...
                if (response.data.Result[i].Hide == false) {
                    ...
                    if (response.data.Result[i].Alias == "document") {
                        ...
                        else if (suffix == "txt" || suffix == "odt" || suffix.includes("doc")) {
                        ...
                        else if (suffix.includes("pp") || suffix.includes("tif") || suffix.includes("odp")) {
                        ...
                        else if (response.data.Result[i].HiddenParent == true) {
                            var url = response.data.Result[i].Url;
                            var newPath = "";
                            if (url.substring(url.length - 1) == "/") {
                                newPath = url + "?overview=v";
                            }
                            else {
                                newPath = url + "/?overview=v";
                            }
                        ...
                        if (response.data.Result[i].MetaTitle != null && response.data.Result[i].MetaTitle != "") {
                        ...
                        if (response.data.Result[i].MetaDescription != null && response.data.Result[i].MetaDescription != "") {
                            desc.textContent = response.data.Result[i].MetaDescription;
                        }
                        else if (response.data.Result[i].Description != "" && response.data.Result[i].Description != null) {
                            desc.textContent = response.data.Result[i].Description;
                        }
                        else if (response.data.Result[i].Summary != "" && response.data.Result[i].Summary != null) {
                            desc.textContent = response.data.Result[i].Summary;
                        }

                        if (response.data.Result[i].Role != "" && response.data.Result[i].Role != null) {
                            var role = document.createElement('p');
                            role.textContent = response.data.Result[i].Role;
                            listInnerContainer.appendChild(role);
                        }
                        if (response.data.Result[i].DirectNumber != "" && response.data.Result[i].DirectNumber != null) {
                            var dn = document.createElement('p');
                            dn.textContent = response.data.Result[i].DirectNumber;
                            listInnerContainer.appendChild(dn);
                        }
                        if (response.data.Result[i].GroupNumber != "" && response.data.Result[i].GroupNumber != null) {
                            var gn = document.createElement('p');
                            gn.textContent = response.data.Result[i].GroupNumber;
                            listInnerContainer.appendChild(gn);
                        }
                        if (response.data.Result[i].Email != "" && response.data.Result[i].Email != null) {
